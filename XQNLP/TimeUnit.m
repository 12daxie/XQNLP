...
- (void)normSetHour {
    ...
    match = [self p_matchTimeExpressionWithRule:@"早上|早晨|早间|晨间|今早|明早|早"];
    if (match.count > 0) {
    ...
    match = [self p_matchTimeExpressionWithRule:@"晚上|夜间|夜里|今晚|明晚|晚"];
    if (match.count > 0) {
        if ([_tp.tunit[3] integerValue] >= 1 && [_tp.tunit[3] integerValue] <= 11)
            _tp.tunit[3] = @([_tp.tunit[3] integerValue] + 12);
        else if ([_tp.tunit[3] integerValue] == 12)
            _tp.tunit[3] = @(0);
        if ([_tp.tunit[3] integerValue] == -1) /**增加对没有明确时间点，只写了“晚上|夜间|夜里|今晚|明晚|晚”这种情况的处理  @author kexm*/
...
- (void)normSetTotal {
    ...
    match = [self p_matchTimeExpressionWithRule:@"(晚)|(晚上)"];
    if (match.count > 0) {
        if ([_tp.tunit[3] integerValue] >= 1 && [_tp.tunit[3] integerValue] <= 11)
            _tp.tunit[3] = @([_tp.tunit[3] integerValue] + 12);
        else if ([_tp.tunit[3] integerValue] == 12)
...
- (void)normSetBaseRelated {
    ...
    bool flag[] = {false, false, false,false,false,false};//观察时间表达式是否因当前相关时间表达式而改变时间
   
    // 秒
    NSArray<NSTextCheckingResult *>* match = [self p_matchTimeExpressionWithRule:@"\\d+(?=秒(以|之)?前)"];
    
    if (match.count > 0) {
        flag[5] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        date = [date ng_fs_dateBySubtractingSeconds:day];
    }
    
    match = [self p_matchTimeExpressionWithRule:@"\\d+(?=秒(以|之)?后)"];
    
    if (match.count > 0) {
        flag[5] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        date = [date ng_fs_dateByAddingSeconds:day];
    }
    
    
    // 半小时
    match = [self p_matchTimeExpressionWithRule:@"(半小时)(以|之)?前"];
    
    if (match.count > 0) {
        flag[4] = true;
        date = [date ng_fs_dateBySubtractingMinutes:30];
    }
    match = [self p_matchTimeExpressionWithRule:@"(半小时)(以|之)?后"];
    if (match.count > 0) {
        flag[4] = true;
        date = [date ng_fs_dateByAddingMinutes:30];
    }
    
    // 分钟
    match = [self p_matchTimeExpressionWithRule:@"\\d+(?=(分|分钟)(以|之)?前)"];
    
    if (match.count > 0) {
        flag[4] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        date = [date ng_fs_dateBySubtractingMinutes:day];
    }
    
    
    match = [self p_matchTimeExpressionWithRule:@"\\d+(?=(分|分钟)(以|之)?后)"];
   
    if (match.count > 0) {
        flag[4] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        
        date = [date ng_fs_dateByAddingMinutes:day];
    }
    
//
    match = [self p_matchTimeExpressionWithRule:@"[13一三](?=刻钟(以|之)?前)"];
    
    if (match.count > 0) {
        flag[4] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        date = [date ng_fs_dateBySubtractingMinutes:day*15];
    }
    
    
    match = [self p_matchTimeExpressionWithRule:@"[13一三](?=刻钟(以|之)?后)"];
    
    if (match.count > 0) {
        flag[4] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        date = [date ng_fs_dateByAddingMinutes:day*15];
    }
    
// 小时
     match = [self p_matchTimeExpressionWithRule:@"\\d+(?=小时(以|之)?前)"];
    if (match.count > 0) {
        flag[3] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        date = [date ng_fs_dateBySubtractingHours:day];
    }
    
    match = [self p_matchTimeExpressionWithRule:@"\\d+(?=小时(以|之)?后)"];
    if (match.count > 0) {
        flag[3] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
        date = [date ng_fs_dateByAddingHours:day];
    }

    
//    天
    match = [self p_matchTimeExpressionWithRule:@"\\d+(?=天(以|之)?前)"];
    if (match.count > 0) {
        flag[2] = true;
        NSInteger day = [[self.timeExpression substringWithRange:match[0].range] integerValue];
...
//    年
    ...
    if (flag[3] || flag[4] || flag[5]) {
        _tp.tunit[3] = @([date ng_fs_hour]);
    }
    if (flag[4] || flag[5]) {
        _tp.tunit[4] = @([date ng_fs_minute]);
    }
    if (flag[5]) {
        _tp.tunit[5] = @([date ng_fs_second]);
    }
}
...
- (void)p_preferFuture:(NSInteger)checkTimeIndex {
    ...
    if ([_tp.tunit[checkTimeIndex] integerValue] != -1) {
        ...
        date = [date ng_fs_dateByAddingValue:[_tp.tunit[checkTimeIndex] integerValue] byIndex:checkTimeIndex];
        for (int i = 0; i <= checkTimeIndex; i++) {
            _tp.tunit[i] = @([date ng_fs_valueBy:i]);
        }
    }
